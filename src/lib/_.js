...
export default function _(rawo) {
	_.all.push(this);
	...
	for (let i = 0; i < this.stops.length; i++) {
		const forStop = this.stops[i] = new _.ColorStop(this, this.stops[i]);
	...
	paint: function() {
		...
		for (let i = 0; i < 360; i += 0.5) {
			if (i / 360 + ε >= stop.pos) {
				...
				sameColor =
					String(prevStop.color) === String(stop.color) &&
					prevStop !== stop;

				diff = prevStop.color.map((c, x) => stop.color[x] - c);
			}
...
_.ColorStop.colorToRGBA = function(color) {
	if (!Array.isArray(color)) {
		return colorTool(color).rgb().array();
	}
